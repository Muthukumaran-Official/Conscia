...
def display_banner():
    try:
        # Go up from tools/ to project root, then into resources/
        project_root = os.path.dirname(os.path.dirname(os.path.realpath(sys.argv[0])))
        banner_path = os.path.join(project_root, "john4py", "john4py_banner.txt")
        ...
        print(colored("[!] Banner file not found in resources folder.", "red"))
...
def main():
    try:
        ...
        project_root = os.path.dirname(os.path.dirname(os.path.realpath(sys.argv[0])))
        wordlist = os.path.join(project_root, "john4py", "rockyou.txt")
